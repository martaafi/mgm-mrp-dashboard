...
Please provide a clear, structured, actionable Industrial Engineering recommendation formatted in markdown with bullet points. Focus on real garment manufacturing techniques (buffer inventory borrowing, operator multi-skilling, workstation layout changes, or preventative maintenance timing).`;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white border border-slate-200 rounded-2xl shadow-2xl max-w-3xl w-full h-[600px] flex flex-col overflow-hidden my-auto animate-in fade-in zoom-in-95 duration-200">
        {/* Modal Header */}
        ...
        <div className="bg-slate-50 px-6 py-2.5 border-b border-slate-200 flex items-center space-x-2 overflow-x-auto text-xs">
          <span className="text-slate-500 shrink-0 font-medium">IE Presets:</span>
          <button
            onClick={() =>
              handleSendMessage(
            ...
            className="px-2.5 py-1 rounded-full bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 shrink-0 transition-colors"
          >
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-full bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 shrink-0 transition-colors"
          >
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-full bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 shrink-0 transition-colors"
          >
            Maintenance Tips
          </button>
        </div>
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-4">
          ...
          {isLoading && (
            <div className="flex items-center space-x-3 text-slate-500 text-xs">
              <div className="w-8 h-8 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center text-indigo-600">
        ...
        <form
          ...
          className="bg-slate-50 px-4 py-3 border-t border-slate-200 flex items-center space-x-2"
        >
          <input
            type="text"
            ...
            disabled={isLoading}
            className="flex-1 bg-white border border-slate-200 rounded-xl px-4 py-2 text-xs sm:text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:border-indigo-600 disabled:opacity-50 shadow-inner"
          />
          <button
            type="submit"
